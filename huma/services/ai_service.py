...
async def _build_agenda_text(identity, conv) -> str:
    ...
    if status == "ok" and snapshot.get("slots"):
        ...
        text = (
            "AGENDA REAL DO CLIENTE (consulta read-only do Google Calendar):\n"
            f"  Horários LIVRES nos próximos dias: {slots_text}.\n"
            "\n"
            "  QUANDO USAR: este bloco é REFERÊNCIA pra QUANDO o lead pedir agendamento\n"
            "  (perguntar disponibilidade, dar urgência, escolher um dia, fechar horário).\n"
            "  Se a conversa AINDA está em rapport, descoberta, apresentação ou objeção,\n"
            "  IGNORE este bloco e siga a psicologia normal da stage — NÃO mencione horário,\n"
            "  NÃO ofereça slot, NÃO 'puxe' agendamento. Espere o lead sinalizar interesse.\n"
            "  REGRA: a IA nunca abandona rapport/perguntas pra empurrar horário.\n"
            "\n"
            "  QUANDO o lead pedir agendamento:\n"
            "    - Ofereça 2-3 opções DESTA lista que casem com a preferência dele.\n"
            "      NUNCA invente horário fora da lista. NUNCA diga que algo está 'ocupado'\n"
            "      se não estiver fora da lista.\n"
            "    - Interprete a preferência com lógica, não casamento de palavra. Exemplos\n"
            "      ILUSTRATIVOS (não lista fechada): 'não posso segunda', 'nem terça nem\n"
            "      quinta', 'só de manhã', 'à tarde', 'depois das 18h', 'o quanto antes',\n"
            "      'essa semana', 'semana que vem'. Vale QUALQUER dia ou expressão que o\n"
            "      lead usar — é o mesmo raciocínio com outro valor.\n"
            "    - Se nada casar exatamente, diga com honestidade o que há de mais próximo\n"
            "      e pergunte se serve.\n"
        ...
        text = (
            ...
            "  QUANDO USAR: SÓ se o lead pedir agendamento. Em conversa normal (rapport,\n"
            "  descoberta, apresentação), siga a psicologia da stage — não mencione agenda.\n"
            "  Se (e SÓ SE) o lead pedir agendamento: avise com honestidade que está cheio\n"
            "  e pergunte se ele consegue mais pra frente. NUNCA invente horário."
        )
    else:

# ================================================================
# huma/core/web_channel.py — Balcão HUMA (canal web) v1
#
...
#
# Decisões de design (estudo de canais 2026-08-16):
...
_BR_PHONE_RE = re.compile(
    r"(?:\+?55[\s.\-]?)?\(?([1-9][0-9])\)?[\s.\-]?(9?[0-9]{4})[\s.\-]?([0-9]{4})"
)
...
async def process_web_message(client_id: str, session_id: str, text: str) -> dict:
    """
    ...
    rate limit → billing → Tier 0 (regras, custo zero) → IA (tiers 2/3)
    → funil/lead_facts → deflection → persistência.
    ...
    if not is_valid_session_id(session_id) or not text:
        return {"status": "invalid", "reply_parts": []}

    phone = web_phone(session_id)

    if await cache.is_duplicate(phone, text):
        return {"status": "ok", "reply_parts": []}

    if not await cache.check_rate_limit(phone):
        return {"status": "rate_limited", "reply_parts": []}
    ...
    try:
        # Import tardio: orchestrator importa muitos serviços; web_channel
        # reusa os caches e helpers dele sem criar ciclo no import do app.
        ...
        if not client_data:
            return {"status": "unavailable", "reply_parts": []}
        ...
        # Billing: conversa web debita crédito igual à do WhatsApp (o
        # custo de LLM é o mesmo). Janela 24h reusa a chave conv_window.
        if await _is_new_conversation(client_id, phone):
            ...
            if not conv_check["has_conversations"]:
                ...
                return {"status": "ok", "reply_parts": [_PAUSED_REPLY]}
        ...
        # mensagem pro humano ter contexto e avisa o visitante.
        if conv.handoff_status == "handed_off":
            conv.history.append({"role": "user", "content": text})
            conv.last_message_at = datetime.utcnow()
            await db.save_conversation(conv)
            return {"status": "ok", "reply_parts": [_HANDOFF_REPLY]}
        ...
        if (classification.can_resolve_without_llm
            ...
            log.warning(f"Limite IA (web) | {phone} | max={max_ia}")
            ai_result = None
        ...
        if ai_result is None:
            # Teto de IA no dia: encerra educadamente sem loop robótico.
            conv.history.append({"role": "user", "content": text})
            conv.last_message_at = datetime.utcnow()
            await db.save_conversation(conv)
            return {"status": "ok", "reply_parts": [_HANDOFF_REPLY]}
        ...
        # Mesmo contrato do attribution_service: grava uma vez, nunca sobrescreve.
        if not conv.lead_source and len(conv.history) <= 2:
        ...
        return {"status": "ok", "reply_parts": reply_parts}

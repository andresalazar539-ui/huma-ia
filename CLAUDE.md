...
- **Integrações nativas "de 1 clique" (2026-09-05)**: migration `scripts/migration_integracoes_nativas.sql` (colunas em `clients`, todas `DEFAULT ''`). Tela Cockpit → Integrações (`IntegrationsScreen.jsx`), status em `GET /api/integrations/status` (só marcadores, nunca segredo), disconnect genérico em `POST /api/integrations/{id}/disconnect`. Páginas de fim de OAuth em `routes/_oauth_pages.py` (voltam sozinhas pro Cockpit).
  ...
  - **Integração conectada DEPOIS do onboarding = efeito imediato (regra do André, 2026-09-07)**: conectar tem que ensinar a IA na hora, igual ao que o onboarding faria; nunca deixar a IA rodando com o conhecimento de antes da integração. Implementado pra Nuvemshop em `huma/core/catalog_sync.py` (puro): o callback grava o catálogo em `products_or_services` (itens com `"source": "nuvemshop"`, teto `MAX_STORE_ITEMS`, sem estoque no prompt) e liga `sell_physical` + `enable_payments`; o disconnect remove só os itens da loja e mantém os do dono. Falha no sync não desfaz a conexão (token já gravado; a página avisa). Toda integração nova deve seguir o mesmo padrão no connect. A tela de Configurações do negócio libera "Vender produto físico" com loja OU Bling (`needs: 'store'`).

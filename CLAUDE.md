...
- **Assinatura: conversa só entra com cobrança APROVADA (2026-09-17, incidente da assinatura de teste de 14/08)**: no Mercado Pago, preapproval `authorized` = cartão válido, NÃO pago; a primeira cobrança do motor de recorrência acontece ~1h depois e pode ser recusada (retry por dias, depois o MP pausa sozinho). **PAGOU, USOU (2026-09-18, exigência do André)**: o Cockpit manda DOIS tokens do mesmo cartão (token do MP é de uso único) + `payment_method_id` do BIN; `create_subscription_with_card` cria o preapproval com `auto_recurring.start_date` = mesmo dia do mês seguinte (`_next_cycle_start_iso`, só renovação) e `_activate_paying_now` cobra a 1ª mensalidade AGORA via `POST /v1/payments` (síncrono, ext_ref `humasub|…`, `metadata.kind=primeira_mensalidade`): aprovado → ativa + credita `mp_primeira_cobranca` com `payid=… pre=…` (o webhook topic payment cai no dedup por payid) + boas-vindas/indicação/purchase no mesmo instante; recusado ou erro → `_abort_preapproval` (PUT cancelled) e erro em PT (`_CARD_DECLINE_PT`), nada gravado; em análise → ativa sem crédito (`awaiting_first_charge`, webhook credita). NUNCA usar o marcador legado `primeira pre=` nessa descrição (`_first_charge_covered` absorveria a renovação do mês 2). Sem os campos novos vale o caminho legado (só preapproval, crédito no webhook ~1h). O crédito das renovações, as boas-vindas (`_ever_paid` decide a 1ª cobrança da vida), a indicação e o purchase server-side saem no webhook da cobrança aprovada (`_handle_authorized_payment` / `credit_subscription_charge`). `_upsert_subscription(..., welcome=False)` espelha status sem boas-vindas. **Dedup de cobrança é EXATA, pelo id do pagamento** (`payid=` na descrição do razão, gravado pelos dois formatos de webhook; `_already_credited(client, apid, pay_id)`); a janela de 20 dias (`_recently_credited`) é só último recurso sem payid, e no topic payment olha só créditos sem payid (`unidentified_only=True`) — senão a primeira cobrança de uma assinatura nova feita dias após uma renovação era engolida (cobrar 2x, creditar 1x). **Webhook de preapproval tem guarda por id**: `pending`/`paused`/`cancelled` de um preapproval que NÃO é o vigente (`payment_provider_id`) nunca rebaixa trial/active (é o antigo cancelado na troca de cartão ou um checkout abandonado); `authorized` de id novo sempre entra. Alerta ao dono (`_notify_payment_problem_bg`: WhatsApp + `email_service.send_payment_problem`) só na transição `active→paused/cancelled` DO MESMO preapproval; `cancel_subscription` grava `cancelled` local ANTES do PUT no MP (volta pra `active` se o MP recusar) justamente pra não gerar alerta falso. Primeira recusa do cartão (`authorized_payment` com `payment.status` rejected) avisa o dono na hora, 1x a cada 3 dias (Redis `sub_reject_alert:{client_id}`). Assinar de novo: foto da linha anterior ANTES do POST (`_current_subscription`), upsert do novo, depois cancela o antigo no MP (`_cancel_previous_preapproval`) e carrega excedente já programado pro novo (`_carry_overage_forward`). `get_billing_status` devolve `awaiting_first_charge` POR PREAPPROVAL (`_preapproval_charged`, `pre=<id>` na descrição). Não reintroduzir crédito na resposta do checkout. **Reconciliação (2026-09-18)**: job `subscription_reconcile` (scheduler, 6h) → `reconcile_subscriptions` confere cada assinatura real no MP (`GET /preapproval/{id}` + `GET /authorized_payments/search?preapproval_id=`), espelha status divergente por `_handle_preapproval_change` e credita cobrança aprovada ausente do razão por `_handle_authorized_payment` (ambos idempotentes) — o webhook virou só latência. **Trocar cartão (2026-09-18)**: `update_subscription_card` faz `PUT /preapproval/{id}` com `card_token_id` + `status=authorized` no MESMO preapproval (valor/cupom/ciclo preservados; reativa pausada), espelha com `welcome=False`, nunca credita, limpa `sub_reject_alert`. Rota `POST /api/clients/{id}/billing/update-card`; `get_billing_status.can_update_card`; Cockpit: botão "Trocar cartão" em Uso abre a `CheckoutScreen` com `ctx.mode='update_card'` (mesma tokenização do SDK do MP). Avisos de cartão recusado apontam pra esse botão, não pra "assinar de novo" (cancelar+recriar fica só como fallback quando o dono escolhe assinar de novo). **Janela "aguardando" no gate (2026-09-18)**: `billing.get_gate_status` devolve `awaiting_first_charge` (aditivo; lê `payment_provider_id` + `_preapproval_charged`, import local pra evitar ciclo) e `resolve_new_conversation` com saldo 0 nessa janela bloqueia em QUALQUER modo (nem excedente) com `reason="awaiting_first_charge"`; `orchestrator._handle_blocked_new_conversation` manda ao dono "processando a primeira cobrança" sem links de gasto extra. Com saldo sobrando (trial, troca de cartão) segue normal.
...
- **Integrações nativas "de 1 clique" (2026-09-05)**: migration `scripts/migration_integracoes_nativas.sql` (colunas em `clients`, todas `DEFAULT ''`). Tela Cockpit → Integrações (`IntegrationsScreen.jsx`), status em `GET /api/integrations/status` (só marcadores, nunca segredo), disconnect genérico em `POST /api/integrations/{id}/disconnect`. Páginas de fim de OAuth em `routes/_oauth_pages.py` (voltam sozinhas pro Cockpit).
  ...
  - **Checkout de Conversa — Etapa 2 (2026-09-08)**: `huma/core/store_checkout.py`. A IA emite `create_store_order` (sku, qty, nome, e-mail, CEP, endereço; linha só entra na tool quando `checkout_enabled(identity)`: loja conectada + `sell_physical` + `store_checkout_shipping` ∈ {gratis, fixo}). `handle_action` confere estoque/preço AO VIVO (`check_stock` por SKU, `variant_id`), soma o frete da política do dono (`store_checkout_shipping[_cents]`, migration `scripts/migration_store_checkout.sql`, editável em Autonomia comercial), manda o resumo + Pix pelo `_handle_payment_action` existente e guarda o rascunho (Redis `store_order_draft:*` 24h + marker `[PEDIDO EM ABERTO {json}]`). `api.handle_payment_result` (aprovado) chama `on_payment_approved` → `NuvemshopAdapter.create_paid_order` (draft order `payment_status=paid` + confirm, nota `HUMA · canal · phone` = atribuição CERTA) → marca `store_order_seen` (o webhook order/paid não conta 2x) → avisa o lead com o nº do pedido; se a loja recusar, avisa o DONO e não promete nada ao lead. Frete NÃO é cotado pela API da Nuvemshop (decisão do dono). Coleta mínima: nome, e-mail, CEP e número — rua/bairro/cidade/UF vêm do ViaCEP (`services/cep_service.py`, cache 30d); o resumo do pedido sai como CARD (`store_checkout.order_card`, `kind: "order"`, sem botões — `generic_template_payload` honra `buttons` explícito) antes do Pix. **Caixinha da HUMA v2 (2026-09-08, exigência do André: UMA página, sem redirect, silêncio na DM)**: quando o lead diz "vou levar", a IA emite `create_store_order` só com sku/qty e o motor manda o card "Finalizar pedido" (`checkout_card`, botão web_url) pra `GET /pedido/{token}` (`routes/store_checkout_page.py`; token Redis `checkout_token:*` 24h via `issue_checkout_link`). A página tem dados (nome, e-mail, CPF, CEP→ViaCEP, número) e pagamento na MESMA tela: `POST /pedido/{token}/pix` (`store_checkout.pay_pix` → MP `/v1/payments` Pix, QR + copia e cola na página) e `POST /pedido/{token}/card` (`pay_card`: cartão crédito/débito tokenizado no navegador pelo SDK do MP com `MERCADOPAGO_PUBLIC_KEY`, cobrança síncrona; aprovado → `handle_payment_result` inline + pedido na loja; recusado → motivo em PT de `_CARD_DECLINE_PT`); `GET /pedido/{token}/status` = poll (pedido criado?). `prepare_order` valida/confere estoque/guarda o rascunho SEM mandar nada pra conversa; a única mensagem ao lead é "Pedido #N criado" (`on_payment_approved`; `handle_payment_result` pula a confirmação genérica quando `has_open_draft`). `handle_payment_result` é idempotente por `payment_done:{id}` (cartão inline + webhook). Registro em `payments` leva `metadata.conversation_phone` (Instagram/site não têm dígitos) e `process_payment_notification` prefere esse campo. Com checkout ligado, cards de produto saem sem "Comprar" e "vou levar" não repete o card de produto. `submit_checkout`/`handle_action` via chat continuam pra quem digita os dados no chat. Etapa 3 (Pix nativo WhatsApp `order_details` + WhatsApp Flows) pendente; canal web não emite actions (E2 não roda no widget).
  - **Mercado Pago DO CLIENTE por OAuth (2026-09-10)**: até aqui todo Pix/boleto/cartão da conversa saía da conta MP DA HUMA (`MERCADOPAGO_ACCESS_TOKEN`, o mesmo token que cobra a assinatura) — o dinheiro do cliente caía na conta da HUMA. Agora `services/mercadopago_oauth.py` + `routes/oauth_mercadopago.py` (`/oauth/mercadopago/start|callback`) conectam a conta do dono (Authorization Code do MP; `platform_id=mp`, redirect estática do painel). Colunas em `clients` (migration `scripts/migration_mercadopago_oauth.sql`): `mercadopago_user_id / nickname / access_token / refresh_token / public_key / token_expires_at / live_mode`; `payment_provider='mercadopago'`. **Resolução do token: `payment_service.mp_token_for(identity)`** (próprio vence, global é fallback) — passado como `access_token=` pra `_mp_post_payment`, `_create_pix/_create_boleto/_create_card`, `check_payment_status` e pela Caixinha (`store_checkout.pay_pix/pay_card`); a página `/pedido/{token}` tokeniza o cartão com `mp_public_key_for(identity)` (token de cartão só vale pra conta dona da public key). **Webhook**: `process_payment_notification(mp_payment_id, mp_user_id=body.user_id)` resolve o token pela ordem registro em `payments` → `db.get_client_by_mercadopago_user_id` → global. Refresh é ROTATIVO (cada renovação devolve refresh novo; `token_updates` grava os dois) e o access vale 180 dias: job `mercadopago_token_refresh` (scheduler, 1x/dia, renova 30 dias antes). Conectar liga `sell_digital` via `effects_for_connect(caps, "payment")`; disconnect limpa e volta pro legado (não revoga no MP). Env: `MERCADOPAGO_OAUTH_CLIENT_ID/SECRET/REDIRECT_URI`. O token global fica pra assinatura da HUMA (`subscription_service`) e como fallback de quem não conectou.
  - **Caixinha UNIVERSAL — Fase 1 de "Compra Sem Sair" (2026-09-10, exigência do André: TikTok Shop, sem link externo, em todo canal)**: a mesma página `/pedido/{token}` fecha QUALQUER venda. `store_checkout.issue_checkout_link(..., origin="store"|"custom", needs_shipping=, description=)` grava a origem no token; `prepare_order` bifurca: `store` = caminho antigo (estoque ao vivo, cupom da loja, draft da Nuvemshop), `custom` (`_prepare_custom`) = serviço/curso/valor combinado, sem estoque, sem endereço quando `needs_shipping=False`, só cupom da conversa. `on_payment_approved` com `origin="custom"` → `_confirm_custom` (UMA confirmação na conversa, marker `[PAGAMENTO CONFIRMADO NA CAIXINHA …]`, `store_order_result` com `paid=True` → `payment_status` devolve `done=True` e a página encerra). **Cartão nunca vira link externo**: `orchestrator._handle_payment_action` com `payment_method="credit_card"` → `_send_caixinha_payment` (card `payment_card` "Pagar", `caixinha_enabled(identity)` = vende + `PUBLIC_BASE_URL` + conta MP; Asaas segue no link próprio); `store_checkout.handle_action` marca `via="store_checkout"` na pay_action pra não redirecionar duas vezes; Pix pedido no chat continua no chat. **O widget VENDE**: `web_channel.build_web_identity` mantém SÓ `sell_*` (agendar/qualificar/áudio seguem fora), regras `_WEB_CHANNEL_RULES_SELLS` (SE/QUANDO), `_web_checkout_cards` transforma `create_store_order`/`generate_payment` em cards `kind="checkout"` e o widget (`chat.html`, `openSheet`) abre a Caixinha em iframe DENTRO da bolha (mesma origem). **Perfil de compra**: `checkout_profile`/`_save_profile` (Redis `checkout_profile:*`, 180d) pré-preenche nome/e-mail/CPF/CEP/número na segunda compra. `caption_for_card` usa o título do botão. Fase 2 (conta HUMA via Asaas BaaS) e Fase 3 (Pix nativo WhatsApp `order_details` + pedido no Bling) pendentes — estudo em https://claude.ai/code/artifact/065560ea-e2c0-4f63-aae5-242756b5e0d3.
  - **Asaas EMBUTIDO na Caixinha — Fase 2a (2026-09-10)**: o Asaas do cliente deixa de mandar link e vira trilho invisível atrás da mesma página. `store_checkout.payment_rail(identity)` = `"asaas"` (payment_provider asaas + `asaas_api_key`) | `"mercadopago"`; `caixinha_enabled` aceita os dois. `providers/payment/asaas.py`: `find_or_create_customer` (GET/POST `/customers` por `cpfCnpj` — cliente do Asaas EXIGE CPF, por isso a página pede CPF nesse trilho), `create_pix_charge` (POST `/payments` PIX + GET `/payments/{id}/pixQrCode` → `encodedImage`/`payload`), `charge_card` (POST `/payments` CREDIT_CARD com `creditCard` + `creditCardHolderInfo` (nome, e-mail, cpfCnpj, postalCode, addressNumber, phone — todos exigidos) + `remoteIp`; parcelas = `installmentCount` + `totalValue`; status CONFIRMED/RECEIVED → approved). **O cartão passa pelo servidor da HUMA só em trânsito**: nunca é logado nem gravado (o registro guarda só bandeira e últimos 4, que o Asaas devolve). `pay_pix`/`pay_card(token, form, remote_ip="")` bifurcam por trilho (`_pay_pix_asaas`/`_pay_card_asaas`); `_record_store_payment(..., provider=)`; `process_asaas_notification` devolve `metadata.conversation_phone` (Instagram/site). Página: `RAIL="asaas"` = sem SDK do MP, campos CEP/número (quando não há entrega) + celular do titular, rodapé "processado pelo Asaas" (exigência regulatória de identificar o processador). Rota `/pedido/{token}/card` passa `X-Forwarded-For` como `remote_ip`. No chat, cliente Asaas com Pix OU cartão recebe o card "Pagar" (Caixinha), nunca link. `_create_asaas_link` fica só como fallback sem `PUBLIC_BASE_URL`. **Revisão 2026-09-10**: `handle_action` com trilho Asaas força a Caixinha mesmo com todos os dados no chat (`force_page`); `_send_caixinha_payment` reaproveita a mesma página por 30 min pro mesmo valor (Redis `caixinha_last:*`); `app.py` liga `EventScrubber(recursive=True)` no Sentry com os campos de cartão/titular — sem isso variáveis locais em erro levariam número de cartão pro Sentry. **A verificar em sandbox**: `installmentCount`+`totalValue` no parcelado e habilitação de cartão na conta do cliente (o Asaas libera cartão via API "sujeito à análise"). Fase 2b (subconta que a HUMA cria, BaaS) pendente do contrato comercial.
...
- **Roteamento do lead qualificado por vendedor (2026-09-10, pedido de vários leads da Octa)**: `huma/core/lead_routing.py` (puro). Quem recebe leads vive em `clients.team_members` (JSONB existente; chaves novas por membro: `phone`, `receives_leads`, `specialty`) — editável no modal "Convidar equipe" do Cockpit (`TeamMemberRow`, `PATCH /api/clients/{id}/team/{email}`; convite também aceita os campos; papel novo `vendedor`). Regra em `_handle_handoff_action`: ninguém marcado → tudo igual (aviso no `owner_phone`); com equipe → `pick_seller`: "Atende" (specialty) casa com resumo+lead_facts (radical sem plural) → essa pessoa; senão rodízio pelo contador Redis `handoff_rr:{client_id}` (Redis off = primeiro da lista). O aviso completo vai pro WhatsApp do vendedor (cabeçalho "pra você, Nome"), o dono recebe UMA linha (`owner_notice`), o lead ouve o nome de quem vai chamar (`final_message`). Falha no WhatsApp do vendedor → cai pro dono (nunca lead no vácuo). Gravação em `Conversation.assigned_to/assigned_name/assigned_at` (migration `scripts/migration_lead_routing.sql`; contrato do `bsuid`: só no upsert quando preenchido, retry sem os campos com WARNING; limpar = `db.set_assignment`, chamado quando o Cockpit devolve a conversa pra HUMA; `/handoff` com `assigned_to` deixa o dono escolher quem fica). Lista do Cockpit e relatório fazem `select` com `assigned_name` e refazem sem ela se a coluna não existir. Relatório: `qualificacao.por_vendedor` + linha "👥 Por vendedor". Webhook `lead.qualified` leva `assigned_to/assigned_name` em `data`. NÃO existe fila/SLA/atribuição por atendente fora do handoff — é só quem RECEBE o lead qualificado.
- **Permissões por papel na equipe (2026-09-10)**: `huma/core/permissions.py` (puro) é a fonte única: papéis `dono | admin | vendedor | recepcao | equipe(legado)` × permissões `conversas | relatorios | faturamento | disparos | ajustes | equipe` (`ROLE_PERMISSIONS`), mapa rota→permissão (`permission_for(method, path)`; GET `/settings`, GET `/team` e GETs de status são livres) e mapa tela→permissão (`SCREEN_PERMISSIONS`). **Quem barra é o middleware `enforce_team_permissions` em `app.py`**: só age em sessão por cookie cujo token carrega e-mail de MEMBRO (não dono); Bearer api_key, sessão antiga sem e-mail e rotas fora do mapa passam direto; nega com 403 em PT (`denied_message`). O token de sessão ganhou o e-mail (`create_session_token(client_id, email=)`, `session_actor(token) -> (client_id, email)`; formato antigo de 3 partes continua válido = dono). `/cockpit` injeta `window.HUMA_ROLE/HUMA_PERMS/HUMA_SCREEN_PERMS`; `Sidebar.jsx` (`filterAllowed`) esconde telas e `Cockpit.html` mostra "Essa área não faz parte do seu papel". `TEAM_ROLE_LABELS` em `business.py` aponta pra `permissions.ROLE_LABELS`; papel editável em `PATCH /team/{email}`. **Convite de equipe = UM e-mail, e é um convite**: `_send_invite` manda `send_team_invite(action_url=/convite/<token>)` (token HMAC `create_invite_token`, 7 dias, prefixo `inv` pra nunca valer como sessão) com botão "Aceitar convite". A página `GET /convite/{token}` (`auth_login.py`) oferece "Aceitar com Google" (mesmo OAuth do login; acesso vem do e-mail estar em `team_members`), "Já tenho senha" (POST /auth/login) e "Criar senha" → `POST /auth/invite/accept` que, servidor-a-servidor no GoTrue, faz ensure_user → `generate_link` (recovery, pega `hashed_token`) → `/verify` → `PUT /user` senha → cookie HUMA. Zero e-mail do Supabase no convite; convite de quem saiu da equipe responde 410. Não introduzir regras de permissão fora do `permissions.py`.
- **Sem travessão em texto pro usuário (André, 2026-09-10)**: e-mails (`email_service.py`, `ops/supabase_email/*.html` + assuntos em `apply_email_config.py`), títulos de página e mensagens de UI não usam "—". Vale o mesmo que já valia pra resposta da IA. Templates do Supabase precisam ser reaplicados com `ops/supabase_email/apply_email_config.py` (exige `SUPABASE_ACCESS_TOKEN`).
...
- **Filtros da aba Conversas (2026-09-17)**: `GET /api/conversations` aceita, além de `filter` (status), `channel` (whatsapp|instagram|web), `assignee` (`huma` | `dono` | e-mail de quem está com o lead) e `date_from`/`date_to` (yyyy-mm-dd em horário de Brasília, sobre `last_message_at`, fim inclusivo). A regra vive em `huma/core/conversation_filters.py` (puro): status/canal/atendente filtram em memória (`apply_filters`), a janela de datas entra NA QUERY (gte/lt) em `db.list_conversations_for_cockpit`; `dono` também pega o lead que o dono atribuiu a si mesmo (`owner_email`). A lista devolve `assigned_to`. Front: `ConversationsData.toConversationQuery` (estado `period|from|to|channel|assignee`), busca é LOCAL (`conversationMatches`: nome, telefone cru, prévia, sem acento), equipe do filtro "Quem atende" vem de `GET /team`. Sem params = comportamento antigo.
- **Quadro de conversas / kanban (2026-09-17)**: dentro de Conversas, alternância Lista | Quadro (`ViewToggle`, lembrada em `localStorage` por cliente). `KanbanBoard.jsx` desenha as 6 etapas do funil (`STAGE_COLUMNS`: discovery→Descobrindo, offer→Negociando, closing→Fechando, committed→Compromissado, won→Fechado, lost→Perdido) com as MESMAS conversas da lista (filtros e busca valem nas duas visões via `useVisibleConversations`). Cartão mostra o que a HUMA já sabe: `lead_facts` (3) e `lead_hints` (objeção ativa, sinal de compra, pressa, do `lead_state`), devolvidos por `GET /api/conversations`; "parado há X" quando etapa aberta sem mensagem há 24h+. Arrastar (desktop) ou menu "Mover para" (celular) chama `POST /api/conversations/{id}/{phone}/stage` (`{stage}`), otimista com Desfazer (toast 6s) e reversão em falha. **O dono pode ir pra qualquer etapa, inclusive `won`** (venda fora do sistema; marca cliente `reason=manual`) e `lost`; a regra "won só o sistema" é pra IA. Etapas válidas em `conversation_filters.STAGES`. Clicar num cartão abre a conversa numa gaveta à direita (desktop) ou tela cheia (celular).
- **Conversa do Cockpit idêntica à do lead (2026-09-10, regra do André)**: o histórico guarda TUDO que o lead recebeu, e a tela de Conversas desenha igual ao canal. Contrato das chaves extras na entrada do histórico (o prompt lê só `role`+`content`; `content` nunca vazio): `cards` (produto/carrossel, `kind` checkout = "Finalizar pedido"/"Pagar", `kind` order = resumo do pedido), `image_url`/`video_url`/`file_url` (foto, vídeo, PDF do boleto), `audio_url`+`audio_text` (áudio da HUMA em `assistant`; áudio do lead em `user`), `parts` (balões). Peças: `orchestrator._record_sent(conv, content, **extra)` pra tudo que a HUMA manda fora da resposta principal (texto do Pix/copia e cola/boleto, mídia do catálogo, fallbacks em texto); marcadores `[CAIXINHA ENVIADA…]` carregam `cards` quando o canal desenhou o card; `services/lead_media.py` sobe áudio/foto do lead pro Storage (balde `audios`, `lead/<client_id>/…`, teto 8 MB, nunca levanta) no webhook (`api._ingest_media_message`) e deixa a URL pendente no Redis (`lead_media:{client}:{phone}`, 15 min) até `lead_media.user_entry` gravar a mensagem do lead nas três entradas `user` do orchestrator. Cockpit: `ConversationsData.mapHistory` (entrada com card/mídia nunca é filtrada como marcador) + `ConversationView.MessageCard`. Ao criar QUALQUER envio novo pro lead, gravar no histórico o que ele viu — senão a conversa do dono fica diferente.
- **Onboarding: análise de mercado em segundo plano (2026-09-20)**: `POST /onboarding/{id}/compile` grava só a compilação das respostas + status SANDBOX e devolve `market_analysis_status="running"` (~15-25s); `analyze_market` (que levava ~95s dos ~110s medidos em prod, com o dono achando que travou) roda em task de background (`routes/onboarding.py::_run_market_analysis`), que RELÊ o cliente depois da chamada de IA antes de aplicar `custom_rules/tone/forbidden_words/market_analysis` (o dono pode editar no playground nesse meio-tempo). Guarda anti-repetição em memória + Redis `onboarding:{client_id}:market_running` (5 min, não liberada de propósito: teto de custo se a IA estiver falhando). Auto-cura: `GET /state` reagenda quando já compilou e `market_analysis` está vazio (deploy no meio da task). Tela: as frases da espera rodam por relógio, então nenhuma afirma que acabou; o "pronto" vem do `done` do `AnalysisCore` quando o servidor responde. O playground avisa "ainda estou estudando seu mercado" via `has_market_analysis`.
